        ...
        postRef.addValueEventListener(object : ValueEventListener
        {
            override fun onDataChange(p0: DataSnapshot)
            {
                if (p0.exists())
                {
                    ...
                    {
                        Glide.with(mContext).load(post.getPostvideo()).centerInside().placeholder(R.drawable.ic_round_play_circle_filled_24).into(imageView)
                    }
                    else
                    {
                        Glide.with(mContext).load(post.getPostimage()).centerInside().placeholder(R.drawable.profile).into(imageView)
                    }
